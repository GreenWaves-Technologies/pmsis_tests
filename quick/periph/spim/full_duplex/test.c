...
static int test_entry()
{
  ...
  for (int i=0; i<NB_BUFFERS; i++)
  {
    set_spim_verif_command(&spim, 0x3, buffer_size*i, buffer_size, cmd_buffer[i], pi_task(&cmd_event[i]));
    pi_spi_transfer_async(&spim, tx_buffer+buffer_size*i, rx_buffer+buffer_size*i, buffer_size*8, PI_SPI_CS_AUTO, pi_task(&buf_event[i]));
  }
  ...
  {
    int mode = i == NB_BUFFERS - 1 ? PI_SPI_CS_AUTO : PI_SPI_CS_KEEP;
    pi_spi_transfer_async(&spim, tx_buffer+buffer_size*i, rx_buffer+buffer_size*i, buffer_size*8, mode, pi_task(&buf_event[i]));
  }
